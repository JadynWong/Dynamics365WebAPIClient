...
namespace Dynamics365WebApi.Auth
{
    public class DynamicsAuth
    {
        ...
        /// <returns></returns>
        public static DynamicsAuth AcquireToken(string ADFS_Uri, string resource, string clientId, string redirectUri,
                    ...
                    using (var response = httpClient.PostAsync(url, new FormUrlEncodedContent(list)).Result)
                    {
                        ...
                        using (var response2 = httpClient.GetAsync(response.Headers.Location).Result)
                        {
                                ...
                                httpClient.PostAsync(tokenUrl, new FormUrlEncodedContent(tokenParams)).Result)
                            {
                                ...
                                var auth = JsonConvert.DeserializeObject<DynamicsAuth>(json);
                                if (auth == null)
        ...
        /// <returns></returns>
        public static DynamicsAuth CrmRefreshToken(string ADFS_Uri, string refresh_token)
        {
            var tokenUrl = BuildTokenUrl(ADFS_Uri);
